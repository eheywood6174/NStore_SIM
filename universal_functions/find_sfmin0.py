...
For values of x below the root, y is positive.
For values of x above the root, y is negative.
...
def find_sfmin0(supply_ca, demand_ca1, storz, NSSm_opts):
    ...
    ttt = sum(svec) - NSSm_opts['accp_umd'] #This is acceptability
    ...
    print(f'<find_sfmin0>: Pt #{idx}: ({x_arr[idx]}, {y_arr[idx]})') #Report first point
    ...
    if ttt<0: #First point was acceptable
        isign = 1 #We are looking for a positive (unacceptable)
        scalx = 1/sf_scl #Set scaling for decreasing
    elif ttt>0: #First point was unacceptable
        isign = -1 #We are looking for a negative (acceptable)
    ...
    while igo==1:
        ...
        [xmat, pmat, lmat, dmat, uvec, cvec, svec] = NStore_sim(
            supply_ca1, demand_ca, storz, nssm_opts_arr)
        ttt = sum(uvec) - NSSm_opts['accp_umd'] #This is acceptability
        ddd = storz[:,2]*storz[:,7] - xmat[-1,:] #Depletion levels
        x_arr[idx] = supply_fctr #Record point (x)
        y_arr[idx] = ttt #Record point (y)
        d_arr[idx,:] = ddd #Store corresponding fill deficits
        print(f'<find_sfmin0>: Pt #{idx}: ({x_arr[idx]}, {y_arr[idx]})') #Report discovered point
        idx += 1 #Increment the count
        ...
        if supply_fctr>=NSSm_opts['sf_max']:
            print("<find_sfmin0>: Supply factor too high")
            ...
            print("<find_sfmin0>: Supply factor too low")
    ...
    #2: There are >0 non-saturated points above the root
    ...
    while igo==1:
        ...
        y_arr[idx] = ttt #Record point (y)
        d_arr[idx,:] = ddd #Store corresponding fill deficits
        print(f'<find_sfmin0>: Pt #{idx} ({x_arr[idx]}, {y_arr[idx]})')
        idx += 1 #Increment the count
        ...
        if iter>=min_IHIt: #We may be eligible to stop
            ...
            flgs1 = y_arr>(0-NSSm_opts['accp_umd']) #Non-saturated actual data
            flgs2 = x_arr>(xa+xc)/2 #Points with x coords. above xbest
            flgs3 = x_arr>0 #Points with x coords. > 0
            flags = flgs1*flgs2*flgs3 #Good points below the root
            num_above = sum(flags) #Count good points above the root
            if num_above>0: #We have >0 good points above the root
        ...
        if iter>=max_IHIt: #Too many iterations, we must stop
            ...
            ii = np.argmin(x_arr[indxA]) #Largest (true) x-coord. < root
            x_arrt = x_arr[indxA]
            xB = x_arrt[ii]
            y_arrt = y_arr[indxA]
            ...
            ii = np.argmax(x_arr[indxB]) #Smallest (true) x-coord. > root
            x_arrt = x_arr[indxB]
            xA = x_arrt[ii]
            y_arrt = y_arr[indxB]
